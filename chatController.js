...
const { uploadBufferParaCloudinary } = require('../config/cloudinary');
...
// =================================================================
// FASE 3/4: Rota Multimodal (Visão) - POST /api/chat/vision
// =================================================================
...
exports.enviarMensagemComImagem = async (req, res) => {
    try {
        ...
        // 1) Sobe o buffer (RAM) da imagem para o Cloudinary -> URL permanente
        const urlImagem = await uploadBufferParaCloudinary(req.file.buffer, `olho-de-rapina/${jogadorAtual}`);

        // 2) Monta o conteúdo multimodal para o Gemini (texto + pixels em base64/inlineData)
        const model = criarModelo(jogadorAtual);
        ...
        // 3) Salva pergunta (com URL da imagem) e resposta no MongoDB
        await Mensagem.create({
            jogador: jogadorAtual,
            role: "user",
            parts: [{ text: perguntaTexto }],
            imagemUrl: urlImagem
        });
        ...
        console.error("❌ Erro na rota de visão:", erro.message);
        return res.status(500).json({ erro: "Falha ao processar a imagem. Tente novamente." });
    }
};

// =================================================================
// Histórico Rico: busca o histórico salvo (texto + imagens) por jogador
// =================================================================
...
exports.buscarHistorico = async (req, res) => {
    try {
        ...
        if (!nickname || !nickname.trim()) {
            return res.status(400).json({ erro: "Informe o nickname para buscar o histórico." });
        }

        const mensagens = await Mensagem.find({ jogador: nickname.trim() })
            .select('role parts imagemUrl dataHora -_id')
            .sort({ dataHora: 1 })
            .lean();

        return res.status(200).json({ sucesso: true, mensagens });
    } catch (erro) {
        console.error("❌ Erro ao buscar histórico:", erro.message);
        return res.status(500).json({ erro: "Erro ao buscar o histórico." });
    }
};
